...
/*************************************************************************
 * file encoding: UTF-8
 * description: draw the contact per month graph
 *         GET: op2=interval
 *        POST: exp=<export case>
 ...
 *************************************************************************/
     ...
     **********************************/
    require(PBR_PATH.'/includes/db/function/contactsmonthcount.php');
    $tRecordset = ContactsMonthCount( CAuth::GetInstance()->GetUsername()
    ...
    if( isset($pCCSV) )
    {
        /** Export case
         **************/
        if( ExportMonthCount( $pCCSV, $tRecordset['values'] )===FALSE )
        {
    ...
    else
    {
        ...
        // Refactoring
        RefactoreMonthCountOrSum( $tRecordset, FALSE );
