...
/*************************************************************************
 * file encoding: UTF-8
 * description: draw the rents' details per year graph
 *         GET: op2=interval
 ...
 *************************************************************************/
     ...
     **********************************/
    require(PBR_PATH.'/includes/db/function/rentsyearsum.php');
    $tRecordset = RentsYearSum( CAuth::GetInstance()->GetUsername()
    ...
    if( isset($pCCSV) )
    {
        /** Export case
         **************/
        if( ExportYearSum( $pCCSV, $tRecordset['values'] )===FALSE )
        {
