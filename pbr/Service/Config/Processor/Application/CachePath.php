...
/**
 * Updates cache path according to the working directory
 */
class CachePath extends Processor
{
    ...
    public function process($settings)
    {
        ...
        $sFormat = extractDepthKeyInArray($settings, ['application' => ['cache_path' => true]]);
        if (! is_string($sFormat)) {
            throw new Exception\MissingSettingException('The application.cache_path setting is missing.');
        }
        $sFormat = trim($sFormat);
        if (strlen($sFormat) == 0) {
            throw new Exception\InvalidSettingException('The application.cache_path setting is not valid.');
        }

        /**
         * Updates.
         */
        $settings['application']['cache_path'] = sprintf($sFormat, $sWorkingDir);
