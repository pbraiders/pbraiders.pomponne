...
class ErrorLog extends Processor
{
    ...
    public function process($settings)
    {
        ...
        $sFormat = extractDepthKeyInArray($settings, ['php' => ['error_log' => true]]);
        if (! is_string($sFormat)) {
            throw new Exception\MissingSettingException('The php.error_log setting is missing.');
        }
        $sFormat = trim($sFormat);
        if (strlen($sFormat) == 0) {
            throw new Exception\InvalidSettingException('The php.error_log setting is not valid.');
        }

        /**
         * Updates.
         */
        $settings['php']['error_log'] = sprintf($sFormat, $sWorkingDir, date("Ymd"));
